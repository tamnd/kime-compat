...
//!
//! The kime repository has this as a `cargo xtask` task because it has an xtask crate for the
//! unsafe rule anyway. Here there is no such crate and no reason to add one, so it is a test.
